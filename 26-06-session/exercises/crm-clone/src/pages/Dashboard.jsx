...
export default function Dashboard() {
  ...
  const criticalStockCount = stockItems.filter(item => item.quantity <= 10).length
  
  const calculatedSales = customers.reduce((sum, c) => c.balance > 0 ? sum + c.balance : sum, 0) + 150000
  ...
  movements.slice(0, 2).forEach(mov => {
    ...
    recentActivities.push({
      ...
      icon: '⚠',
      title: `Stok Hareketi: ${item?.name || mov.sku}`,
      subtitle: `${mov.type} - ${mov.quantity} Adet (${mov.note})`,
      time: mov.date,
  ...
  customers.slice(-2).reverse().forEach(c => {
    recentActivities.push({
      ...
      icon: '✓',
      title: `Müşteri Eklendi: ${c.name}`,
      subtitle: c.company || 'Bireysel Müşteri',
      time: 'Yeni',
    ...
    <div className="tab-content dashboard-content">
      <div className="page-header">
        ...
        <div className="header-actions">
          ...
          <button type="button" onClick={() => dispatch(setActiveTab('customers'))} className="btn-primary">Yeni Ekle</button>
        </div>
      </div>
      ...
      <div className="grid-dashboard">
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-details">
            <span className="kpi-label">Toplam Stok Miktarı</span>
            <span className="kpi-value">{totalStockQuantity.toLocaleString('tr-TR')} Adet</span>
            <span className="kpi-change text-emerald-600">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path>
              </svg>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-details">
            <span className="kpi-label">Kritik Stok Uyarısı</span>
            <span className="kpi-value">{criticalStockCount} Kalem</span>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-details">
            <span className="kpi-label">Hesaplanan Bakiye Hacmi</span>
            <span className="kpi-value">₺{calculatedSales.toLocaleString('tr-TR')}</span>
            <span className="kpi-change text-emerald-600">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path>
              </svg>
              ...
              sortedActivities.map((act, index) => (
                <div key={index} className="activity-item">
                  ...
                  <div>
                    ...
                    <p className="activity-time">{act.subtitle} • {act.time}</p>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-xs text-slate-400">Henüz bir aktivite gerçekleşmedi.</p>
            )}
          </div>
        </div>
      </div>
